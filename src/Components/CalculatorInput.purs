...
import Data.Array ((..))
import Logic.Tokens (Token, addT, mkValueT, multiplyT, squareRootT, subT)
    ...
    pure $ DOM.section
      ...
      , children:
          ...
          , button { onClick: props.runEquals, icon: "=" }
            ...
            <>
              ( 0 .. 9 # map mkValueT # map \token ->
                  button { onClick: props.runAddToken token, icon: show token }
              )

            <>
              ( [ addT, subT, multiplyT, squareRootT ] # map \token ->
                  button { onClick: props.runAddToken token, icon: show token }
              )
      }
